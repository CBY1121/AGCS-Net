...
def thresh_OTSU(path):#path:'assets/octa/pred/'
    for file in glob.glob(os.path.join(path + 'images/' + '*pred.png')):  #    for file in glob.glob(os.path.join(path, '*pred.png')):
        index = os.path.basename(file)[:-4]
        image = cv2.imread(file)
        gray = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
        thresh, img = cv2.threshold(gray, 0, 255, cv2.THRESH_OTSU)
        cv2.imwrite(os.path.join(path+ 'images/', index + '_otsu.png'), img)
        print(file, '\tdone!')
